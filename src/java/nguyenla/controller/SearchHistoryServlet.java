...
public class SearchHistoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int pageSize = 5;
            int countPage = userDAO.countHistorySubject(userSearch,nameUser);
            int endPage = countPage / pageSize;
            if (countPage % pageSize != 0) {
                endPage++;
            }

            List<UserHistoryDTO> listHistory = new ArrayList<>();
            listHistory = userDAO.searchHistorySubject(userSearch, index,nameUser);

            request.setAttribute("endPage", endPage);
            request.setAttribute("searchName", userSearch);
            request.setAttribute("listSearchHistory", listHistory);
            request.setAttribute("historySearch", "search");
